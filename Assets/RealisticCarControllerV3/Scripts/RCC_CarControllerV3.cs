...
//				rigid.AddRelativeTorque (Vector3.up * (((steerInput * _gasInput) * 10000f)), ForceMode.Force); 
	...
	public void ResetCar (bool External=false)
	{
	...
	void OnCollisionEnter (Collision collision){
		...
		if (collision.contacts.Length < 1 || collision.relativeVelocity.magnitude < minimumCollisionForce)
			...
			if(crashClips.Length > 0){
                ...
                if (CollisonCount <= 21 && canHitt)
                {
                    ContactPoint contactP = collision.contacts[0];
                    UIcontrols.Obj.FireObj.transform.GetChild(CollisonCount).transform.position = contactP.point;
                    UIcontrols.Obj.FireObj.transform.GetChild(CollisonCount).gameObject.SetActive(true);
                    canHitt = false;
                    CollisonCount++;
